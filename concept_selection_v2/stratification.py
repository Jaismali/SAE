...
[Copied into concept_selection_v2/ from Concept_selection/stratification.py
for pipeline self-containment, same pattern as contract.py/
monosemanticity_filter.py/deduplication.py. If the canonical
Concept_selection/ copy is ever edited, this copy must be updated too
-- these are not currently kept in sync automatically. Byte-diff
verification between this copy and the Concept_selection/ original has
NOT been run -- flagged as an open item in
PROPOSAL_pipeline_reconciliation.md.]
